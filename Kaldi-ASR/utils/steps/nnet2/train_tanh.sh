...
#online_ivector_dir=exp/make_ivectors/train
...
if [ $stage -le -4 ]; then
  ...
  #steps/nnet2/get_lda.sh $lda_opts "${extra_opts[@]}" --cmd "$cmd" --online-ivector-dir $data $lang $alidir $dir || exit 1;
  steps/nnet2/get_lda.sh $lda_opts "${extra_opts[@]}" --cmd "$cmd" --online-ivector-dir exp/make_ivectors/train $data $lang $alidir $dir || exit 1;
fi
