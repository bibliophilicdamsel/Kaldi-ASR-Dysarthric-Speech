...
# beginning and end.
with open("data/lang_test_bg/phones/nonsilence.txt","r") as fr:
	n_utt= len(fr.readlines())
fr.close()

for boundary_type in 'begin', 'end':
    phone_to_lengths = phone_lengths[boundary_type]
    num_utterances = n_utt + 1 								#optional SIL is also counted as an utterance
...
#############################################################CREATING TRIPHONES###########################################################
#create a phoneset list
with open("data/lang_test_bg/tamil_phoneset.txt","r") as f:
	ps= f.readlines()
ps= [l.strip() for l in ps]
f.close()

with open("data/lang_test_bg/tphones.txt","r") as ft:
	rt= ft.readlines()
rt = [t.strip() for t in rt]
ft.close()

with open("data/lang_test_bg/phones/nonsilence.txt","r") as fr:
	w= fr.readlines()
w= [y.strip() for y in w]
fr.close()

def generate_nphones(word_list,n):
	nphones_list=[]
	for num in range(0,len(word_list)-1):
		nphone= ' '.join(word_list[num:num + n])
		nphones_list.append(nphone)
	return nphones_list
...
for word in rt:
	temp= word.split()
	l1= temp[:int((len(temp)+1)/2)-1]
	l2= temp[int((len(temp)+1)/2):]
	for phone in ps:
		if phone in temp[int((len(temp)+1)/2)]:
			pl.append(phone)
			new_list= l1 + pl + l2
			nsil= generate_nphones(new_list,3)
			tot_triphones.append(nsil)
		else:
			if temp[int((len(temp)+1)/2)] not in w:
				optSil= generate_nphones(temp,3)
				tot_triphones.append(optSil)
				
for one_phone in tot_triphones:
	if one_phone not in t_triphones:
		t_triphones.append(one_phone)
tot_num_phones=0
for triphone in t_triphones:
	for t_phone in triphone:
		tot_num_phones+=len(t_phone)


for boundary_type in 'begin', 'end', 'all':
    phone_to_lengths = phone_lengths[boundary_type]
    #tot_num_phones = total_phones[boundary_type]
    #tot_num_phones= len(tot_phones)
