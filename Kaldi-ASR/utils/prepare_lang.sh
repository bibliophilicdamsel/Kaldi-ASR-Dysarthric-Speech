...
if [[ ! -f $srcdir/lexiconp.txt ]]; then
  ...
  #awk -v FS=' ' -v OFS=' ' '{$2=$2" "1.0} 1' data/local/phoneset.txt > data/local/dict/phoneset.txt
  #sed -i 's/1/1.0/g' data/local/dict/phoneset.txt
  #cat data/local/dict/phones.txt $srcdir/lexicon.txt > $srcdir/lexicon_complete.txt
...
#awk -v FS=' ' -v OFS=' ' '{$2=$2" "1.0} 1' data/local/phoneset.txt > data/local/dict/phoneset.txt
#sed -i 's/1/1.0/g' data/local/dict/phoneset.txt
#cat data/local/dict/phoneset.txt $srcdir/lexicon.txt > $tmpdir/lexiconp_mod.txt
#cut -d' ' -f1 $tmpdir/lexiconp.txt > $tmpdir/words_tmp.txt
#cat $srcdir/phones.txt $tmpdir/words_tmp.txt | sort | uniq  | awk '
...
#num_states=()
#readarray -t num_states< data/local/num_states.txt
#generate nonsilencephoneslist
#nonsilencephoneslist=()
#readarray -t nonsilphonelist< data/local/nonsilence.int

#mkdir $dir/topology

#generate topology
#for ((i=0; i<=778; i++)); do
#	local/gen_topo_nonsil.pl ${num_states[$i]} $num_sil_states  ${nonsilphonelist[$i]} $silphonelist > $dir/topology/topology$i
#done

#ls -v $dir/topology/topology* | xargs cat > $dir/topo
#rm $dir/topology/topology*

#for ((i=0; i<=778; i++)); do
#	utils/gen_topo.pl ${num_states[$i]} $num_sil_states  ${nonsilphonelist[$i]} $silphonelist > $dir/topology/topo$i
#done
