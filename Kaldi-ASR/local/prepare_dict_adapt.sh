...
cut -d' ' -f2- $srcdir/trainadapt_phn.trans | tr ' ' '\n' | sort -u > $dir/phones.txt
cut -d' ' -f2- $srcdir/trainadapt.trans | tr ' ' '\n' | sort -u > $dir/words.txt
...
export PATH=${PATH}:$SRILM/bin/i686-m64
 ...
 # echo "$0: Error: the IRSTLM is not available or compiled" >&2
  ...
  #exit 1
#fi
sed -i 's/\t/ /g' $srcdir/trainadapt.trans
cut -d' ' -f2- $srcdir/trainadapt.trans | sed -e 's:^:<s> :' -e 's:$: </s>:' > $srcdir/lm_train.text
